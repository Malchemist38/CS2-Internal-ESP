    ...
    if (!init) {
        if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D11Device), (void**)&pDevice))) {
            ...
            if (SUCCEEDED(pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pBackBuffer))) {
                ...
                return oPresent(pSwapChain, SyncInterval, Flags); // skip rendering if backbuffer fails
            }
            ...
            return oPresent(pSwapChain, SyncInterval, Flags);  // exit if device creation fails
        }
    }
...
BOOL WINAPI DllMain(HMODULE hMod, DWORD dwReason, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH:
        ...
        break;
    case DLL_PROCESS_DETACH: // gracefully deallocate resources
